...
def parse_files(date):
    ...
    df.rename(columns={'AMC':'amc_name', 'Code':'scheme_code', 'Scheme Name':'fund_name', 'Scheme Category':'fund_category',
                       'Scheme NAV Name':'scheme_name', 'Scheme Minimum Amount':'min_amount', 'Launch Date':'inception_date',
                       'Closure Date':'closure_date', 'ISIN Div Payout/ ISIN GrowthISIN Div Reinvestment':'isin'}, inplace=True)
    ...
    fund_amc_temp['amc_name_closest'] = fund_amc_temp['amc_name'].apply(get_closest_match, args=(amc_master['amc_name'],))
...
def assign_new_fund_ids(df, fund_master, fund_master_file):
    ...
    if nids > 0:
        ...
        df_fund = df_fund[['fund_id', 'fund_name', 'amc_id', 'fund_category', 'inception_date', 'closure_date', 'min_amount']]
...
def assign_new_scheme_ids(df, fund_scheme_master, fund_scheme_master_file):
    ...
    if nids > 0:
        ...
        df_scheme = df_scheme[['scheme_id', 'scheme_code', 'isin', 'scheme_name', 'fund_id', 'plan_type', 'distribution_option']]
