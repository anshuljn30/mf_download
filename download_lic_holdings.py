...
def download(dates, path):
	...
	for d in dates:
		...
		month = d.strftime("%B")
		driver.get(url)
		portfolios = driver.find_element_by_xpath('.//h2[contains(text(), "Monthly") and contains(text(), "Portfolio")]')
		portfolios = portfolios.find_element_by_xpath('.//parent::div').click()
		time.sleep(3)

		file = driver.find_elements_by_xpath('.//a[contains(text(), "' + year + '") and contains(text(), "' + month + '") and (contains(text(), "Portfolio") or contains(text(), "portfolio"))]')
