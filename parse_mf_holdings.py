...
def map_to_ids(df, security_master, fi_master, fund_master):
    ...
    # Map fixed income ids from fi master
    df_null = df_clean[df_clean['security_id'].isnull()]
    df_notnull = df_clean[df_clean['security_id'].notnull()]
    del df_null['security_id']
    df_fi = pd.merge(df_null, fi_master[['isin', 'security_id']], left_on='isin', right_on='isin', how='left')

    # Overwrite is_equity to NaN where security_id is present
    where_not_empty = df_fi[df_fi['security_id'].notnull()].index
    df_clean.loc[where_not_empty, 'is_equity'] = np.nan
    df_clean = df_notnull.append(df_fi)

    # normalize is_equity on all holdings with same isin
    df_clean = normalize_column(df_clean, 'isin', 'is_equity')
...
def cleanup_isin(df):
    ...
    df_clean = normalize_column(df_equity, 'security_name', 'isin')
...
def assign_new_security_ids(df, new_security_master):
    ...
    df_equity = normalize_column(df_equity, 'isin', 'security_name')

    nids = len(df_equity.index)
    if nids > 0:
        df_equity = df_equity.drop_duplicates(subset='isin').reset_index(drop=True)
        nids = len(df_equity.index)
...
def assign_new_fi_ids(df, new_fi_master):
    ...
    df_fi = df_fi[['isin', 'security_name', 'coupon', 'rating', 'maturity']]
    df_fi = df_fi.drop_duplicates()

    df_fi = normalize_column(df_fi, 'isin', 'security_name')
    ...
    if 'maturity' in df_fi.columns:
        df_fi = normalize_column(df_fi, 'isin', 'maturity')
    ...
    if nids > 0:
        ...
        df_fi = df_fi[['security_id', 'security_name', 'isin', 'coupon', 'rating', 'maturity']]
    return df_fi
...
def parse_sheet_data(df, date, sheet_name, header_index, amc_name_spaced):
    ...
    if not coupon_col.empty:
        ...
        df['coupon'] = ""
    maturity_col = df.filter(regex='maturity').columns
    if not maturity_col.empty:
        df.rename(columns={maturity_col[0]: 'maturity'}, inplace=True)
    else:
        df['maturity'] = ""
        ...
        if any((df_temp['isin'].astype(str).str[:2] == 'IN') & (df_temp['isin'].astype(str).str.len() == 12)):
            ...
            if get_des == 1:
                ...
                if matches_debt:
                    ...
                    if matches_listed is not True:
                        is_equity_flag = 0

            # Assign is_equity = 1 if flag is turned on
            if is_equity_flag == 1:
                df_temp['is_equity'] = 1
            else:
            ...
            if any((df_temp['isin'].isnull()) | (df_temp['isin'] == '')):
                # Check if name is also blank, then only turn the get_des on
            ...
            # If there are 2 blank isin in a row then turn get_des on
            if counter > 1:
                get_des = 1
