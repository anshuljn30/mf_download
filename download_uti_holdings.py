...
def download(dates, path):
    ...
    for d in dates:
        ...
        inputs[0].click()
        time.sleep(3)
        inputs[0].find_element_by_xpath('..').find_element_by_xpath(
            './/li/span[contains(text(), "' + year + '")]').click()
        time.sleep(3)
        inputs = block.find_elements_by_xpath('.//input')
        inputs[1].click()
        time.sleep(3)
        inputs[1].find_element_by_xpath('..').find_element_by_xpath(
            './/li/span[contains(text(), "' + month + '")]').click()
        time.sleep(3)
